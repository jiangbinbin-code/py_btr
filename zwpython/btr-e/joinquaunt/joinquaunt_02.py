...
def initialize(context):
    ...
    set_subportfolios([SubPortfolioConfig(cash=0, type='stock'),
                       SubPortfolioConfig(cash=context.portfolio.total_value, type='index_futures')])
...
def set_slip_fee(context):
    ...
    if dt < datetime.datetime(2012, 9, 24):
        ...
        # 手续费
        set_order_cost(OrderCost(open_tax=0, close_tax=0, open_commission=0.00005, close_commission=0.00005,
                                 close_today_commission=0.00005, min_commission=0), type='index_futures')
        # 每日最多交易手数,之前没有交易限制
        g.count = float('inf')
    elif dt < datetime.datetime(2015, 8, 28):
        g.futures_margin_rate = 0.12
        set_order_cost(OrderCost(open_tax=0, close_tax=0, open_commission=0.000025, close_commission=0.000025,
                                 close_today_commission=0.000025, min_commission=0), type='index_futures')
        g.count = float('inf')
    elif dt < datetime.datetime(2015, 9, 6):
        g.futures_margin_rate = 0.4
        set_order_cost(OrderCost(open_tax=0, close_tax=0, open_commission=0.000025, close_commission=0.000025,
                                 close_today_commission=0.00115, min_commission=0), type='index_futures')
        g.count = float('inf')
    else:
        g.futures_margin_rate = 0.4
        set_order_cost(OrderCost(open_tax=0, close_tax=0, open_commission=0.000025, close_commission=0.000025,
                                 close_today_commission=0.0023, min_commission=0), type='index_futures')
        g.count = 5
...
def rebalance(context, signal):
    ...
    f1, f2 = get_current_month_future(context, 'IF'), get_next_month_future(context, 'IF')
    # 此处用了个未来函数，查一下未来成交量
    t1, t2 = get_price(f1, count=1, end_date=context.current_dt, frequency='1m', fields='volume'), get_price(f2,
                                                                                                             count=1,
    ...
    if (len(df_01['close'].dropna()) != 0 and len(df_02['close'].dropna()) != 0):
        t = min(int(t1['volume'].values / 10), int(t2['volume'].values / 10),
                int((context.portfolio.total_value / 2) / (300 * g.futures_margin_rate * df_01['close'].values)),
                int((context.portfolio.total_value / 2) / (300 * g.futures_margin_rate * df_02['close'].values)),
                (g.count))
        print('t:%d' % t)
        # 根据signal值选择建仓或者平仓
...
def get_current_month_future(context, symbol):
    ...
    if dt.day <= third_friday_date or (dt.day > third_friday_date and not any(
            [datetime.date(dt.year, dt.month, third_friday_date + i) in get_all_trade_days() for i in
             range(dt.day - third_friday_date)])):
        year = str(dt.year)[2:]
        month = str(dt.month)
    else:
